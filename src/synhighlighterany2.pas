...
type
  TtkTokenKind = (tkComment, tkIdentifier, tkKey, tkNull, tkNumber,
    ...
    //P.Fradin
    tkMacPredef, tkMacUser, tkVarglob,tkVarPredef
    //end P.Fradin
    );
...
type
  ...
  private
    ...
    fObjects:TStrings;
    //P.Fradin
    fMacPredefAttri: TSynHighlighterAttributes;
    fMacUserAttri: TSynHighlighterAttributes;
    fVarGlobAttri: TSynHighlighterAttributes;
    fVarPredefAttri: TSynHighlighterAttributes;
    //end P.Fradin    
  ...
  published
    ...
    //P.Fradin
    property MacPredefAttri: TSynHighlighterAttributes read fMacPredefAttri write fMacPredefAttri;
    property MacUserAttri: TSynHighlighterAttributes read fMacUserAttri write fMacUserAttri;
    property VarGlobAttri: TSynHighlighterAttributes read fVarGlobAttri write fVarGlobAttri;
    property VarPredefAttri: TSynHighlighterAttributes read fVarPredefAttri write fVarPredefAttri;
    //end P.Fradin
...
uses
  ...
  //P.Fradin
   listes2,
  analyse4, //pour les macros
  graph1_6; //pour IsGraphElem
  //end P.Fradin  
...
begin
  ...
  Result := False;
  Token := AKeyword;//UpperCase(AKeyword); //P.Fradin
  while First <= Last do
  begin
    I := (First + Last) shr 1;
  ...
  result:=result or (fonction(AKeyword)<>nil) or (commande(AKeyword)<>nil) //PFradin
end; { IsKeyWord }


function TSynAnySyn.IsConstant(const AConstant: string): boolean;             //P.Fradin 2008-12-09
var C:Pconstante;
begin
...
function TSynAnySyn.IsObject(const AObject: string): boolean;             //P.Fradin 2008-12-09
begin
  result:=IsGraphElem(Aobject)
end; { IsObject }

function TSynAnySyn.IsMacPredef(const AConstant: string): boolean;             //P.Fradin 2008-12-09
var mac:Pmacros;
begin
...
function TSynAnySyn.IsMacUser(const AConstant: string): boolean;             //P.Fradin 2008-12-09
var mac:Pmacros;
begin
  mac:=macros(AConstant);
  result:= ((mac<>Nil) and (mac^.statut=0))
...
function TSynAnySyn.IsVarGlob(const AConstant: string): boolean;             //P.Fradin 2008-12-09
var V:Pvarglob;
begin
  V:=VarGlob(AConstant);
...
function TSynAnySyn.IsVarPredef(const AConstant: string): boolean;             //P.Fradin 2007-10-28
var V:Pvarglob;
    C:Pconstante;
begin
  V:=VarGlob(AConstant);
...
begin
  for I := #0 to #255 do
    case I of
      ...
      //'(': fProcTable[I] := @RoundOpenProc; //P.Fradin
      //')': fProcTable[I] := @RoundCloseProc;//P.Fradin
...
begin
  ...
  fConstantAttri := TSynHighlighterAttributes.Create('jan_constant');
  fConstantAttri.Style := [fsBold];
  fConstantAttri.Foreground:=clolive;
  AddAttribute(fConstantAttri);

  fObjectAttri := TSynHighlighterAttributes.Create('jan_object');
  fObjectAttri.Style := [fsItalic];
  fObjectAttri.Foreground:=clFuchsia;
  AddAttribute(fObjectAttri);

  fMacPredefAttri := TSynHighlighterAttributes.Create('jan_macpredef');
  fMacPredefAttri.Style := [fsBold];
  fMacPredefAttri.Foreground:=clmaroon;
  AddAttribute(fMacPredefAttri);

  fVarPredefAttri := TSynHighlighterAttributes.Create('jan_varpredef');
  fVarPredefAttri.Style := [fsBold];
  fVarPredefAttri.Foreground:=cldkgray;
  AddAttribute(fVarPredefAttri);

  fMacUserAttri := TSynHighlighterAttributes.Create('jan_macuser');
  fMacUserAttri.Style := [fsItalic];
  fMacUserAttri.Foreground:=clPurple;
  AddAttribute(fMacUserAttri);

  fVarGlobAttri := TSynHighlighterAttributes.Create('jan_varglob');
  fVarGlobAttri.Style := [fsItalic];
  fVarGlobAttri.Foreground:=clnavy;
  AddAttribute(fVarGlobAttri);
  // end P.Fradin
...
//P.Fradin
  else if IsMacPredef(aToken) then fTokenId:=tkMacPredef
  else if IsMacUser(aToken) then fTokenId:=tkMacUser
  else if IsVarGlob(aToken) then fTokenId:=tkVarGlob
  else if IsVarPredef(aToken) then fTokenId:=tkVarPredef
  //end P.Fradin  
...
begin
  case fTokenID of
    ...
    //P.Fradin
    tkMacPredef: Result := fMacPredefAttri;
    tkVarPredef: Result := fVarPredefAttri;
    tkMacUser: Result := fMacUserAttri;
    tkVarGlob: Result := fVarGlobAttri;
    //end P.Fradin    
  else
    Result := nil;
  end;
